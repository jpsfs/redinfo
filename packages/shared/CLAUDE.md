...
| Transport requests | `TransportRequest`, `TransportRequestOccurrenceType`, `TransportRequestVehicleType`, `TransportRequestDecision`, `DestinationFacilityInput`, `validateTransportRequest`, `minutesUntilResponseDue`, `DecideTransportRequestInput` (#228) — a referral entered by hand, field order mirroring the source document; `destinationFacility` is a create-if-missing alternative to `destinationFacilityId` |
