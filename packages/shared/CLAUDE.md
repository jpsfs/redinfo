...
| Week strip | `WeekDateSummary`, `TripsWeekOverview` (#247 stage 6) — the `GET /trips/week?from=` response shape: per-date people/journeys/unplanned/out-of-district/committed-hours counts for the seven days from `from`. `GET /trips/crew/:userId?date=` is this stage's other endpoint but reuses `MyTransportTripsResponse` from Crew manifest above rather than a new shape |
