...
| The delegation's own configuration | `DelegationSettings`-adjacent shapes; `ArrivalWindowThresholds`, `resolveArrivalWindowThresholds`, `arrivalWindowWarning` (#233) — per-facility-overridable soft arrival-timing policy |
...
| Planning board | `TransportPlanningBoard`, `TransportPlanningLane`, `TransportPlanningLeg` (#235) — the `GET /trips/board?date=` response shape: every lane for the date plus the legs still waiting to be dragged onto one, keyed in one `legsById` lookup |
