...
describe('AssignPersonDialog', () => {
  ...
  // Stage 1 of the paid-staff rework removed the paid-extra checkbox from
  // this dialog entirely: compensation is no longer a person-level flag
  // checked at assign time — it is classified per assignment, after the
  // fact, via the coordinator-only compensation dialog on the board.
  it('never renders a paid/compensation control here — that moved to the board', async () => {
    renderDialog();
    await screen.findByText('Bruno Costa');
    expect(screen.queryByText(/paid extra/i)).not.toBeInTheDocument();
