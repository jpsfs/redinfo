...
`src/resources/<name>/` — one dir per CRUD resource (`agreements`, `availability`, `eventReports`,
`facilities`, `inventory`, `liveRuns`, `organisations`, `patients`, `schedules`, `transportRequests`,
`users`, `vehicles`), each with an
