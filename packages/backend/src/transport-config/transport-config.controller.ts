...
import { Action, TransportRequestOccurrenceType, validateArrivalWindowThresholds, validateOccurrenceTypePolicy } from '@redinfo/shared';
 ...
 * and occurrence-type duration floors, both config a coordinator can change
 * without a deploy. Every route gated `MANAGE_TRANSPORT_CONFIG`, the same
 * action `organisations`/`agreements` use for transport reference data.
 *
 * Reuses `DelegationSettingsService` for the thresholds rather than owning a
 * second table — they live on the same `DelegationSettings` singleton row as
 * the base/CODU fields `live-runs` writes; `update` merges a patch so each
 * screen only ever touches the slice it owns.
 */
